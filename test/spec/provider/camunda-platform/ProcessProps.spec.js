...
describe('provider/camunda-platform - ProcessProps', function() {
  ...
  describe('bpmn:Participant#processRef.name', function() {
    ...
    it('should display only one', inject(async function(elementRegistry, selection) {
      ...
      // when
      const processNameInputs = domQueryAll('input[name=processName]', container);

      // then
      expect(processNameInputs).to.have.lengthOf(1);
    }));
  });
