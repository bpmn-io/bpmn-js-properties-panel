...
describe('conditional-properties', function() {
  ...
  describe('editing', function() {
    ...
    var diagramXML = require('./Conditional.bpmn');
    ...
    describe('conditional sequence flows', function() {
    ...
    describe('conditional events', function() {

      it('should fetch the condition of a conditional event',
        inject(function(propertiesPanel, selection, elementRegistry) {
      ...
      it('should change the condition of a conditional event',
        inject(function(propertiesPanel, selection, elementRegistry) {
          ...
          var conditionalEventDefinition = eventDefinitionHelper.getConditionalEventDefinition(shape),
              conditionType = TestHelper.selectedByIndex(domQuery('select[name=conditionType]', propertiesPanel._container)),
              conditionInput = domQuery('input[name="condition"]', propertiesPanel._container);
          ...
          expect(conditionType.value).to.equal('expression');
          expect(conditionInput.value).to.equal('${foo.id()}');
          expect(conditionalEventDefinition.condition.get('body')).to.equal(conditionInput.value);
      ...
      it('should remove the condition of a condition expression conditional event',
        inject(function(propertiesPanel, selection, elementRegistry) {
          ...
          expect(conditionInput.value).to.be.empty;
          expect(conditionInput.className).to.equal('invalid');
          expect(conditionalEventDefinition.condition.get('body')).to.equal('');
        })
      );
      ...
      it('should change condition type from expression to <>',
        inject(function(propertiesPanel, selection, elementRegistry) {
          ...
          expect(conditionType.value).to.equal('');
          expect(conditionInput.parentElement.className).to.contain('bpp-hidden');
          expect(conditionalEventDefinition.condition).to.be.undefined;
        })
      );
