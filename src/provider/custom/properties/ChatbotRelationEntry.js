...
import { getChatbotRelations, subscribeRelationsUpdated, deleteChatbotRelation } from './relationsHelper';

/**
 * Entry component for Chatbot Relation.
 * - Reads from window.__bpmnRelations (injected by BpmnEditor.tsx)
 * - Re-renders when 'bpmn-relations-updated' CustomEvent fires
 * - Trash icon calls deleteChatbotRelation → BpmnEditor.tsx handler → re-inject → event fires
 */
export default function ChatbotRelationEntry(props) {
  const { element } = props;
  ...
  const [items, setItems] = useState(() => getChatbotRelations(elementId));
  const [loading, setLoading] = useState(false);

  // Re-read data whenever BpmnEditor updates window.__bpmnRelations
  useEffect(() => {
    const refresh = () => setItems(getChatbotRelations(elementId));
  ...
  const handleDelete = useCallback(async (callRefId) => {
    setLoading(true);
    try {
      await deleteChatbotRelation(elementId, callRefId);
    } finally {
      setLoading(false);
    }
  }, [elementId]);
  ...
  if (!items || items.length === 0) {
    return html`
      <div class="bio-properties-panel-entry" style="padding: 8px 12px; color: #888; font-size: 12px;">
        ${translate('No chatbot relations')}
      </div>
    `;
  }
  ...
  return html`
    <div style="padding: 4px 0; opacity: ${loading ? '0.6' : '1'}; pointer-events: ${loading ? 'none' : 'auto'};">
      ${items.map((item, idx) => html`
        <div
          key=${'chatbot-' + idx + '-' + item.callRefId}
          style="
