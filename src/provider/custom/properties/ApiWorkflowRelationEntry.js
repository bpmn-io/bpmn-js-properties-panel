...
import { getApiWorkflowRelations, subscribeRelationsUpdated, deleteApiWorkflowRelation } from './relationsHelper';

/**
 * Entry component for API Workflow Relation.
 * Reads from window.__bpmnRelations injected by BpmnEditor.
 * Displays callRefId (API scenario/workflow ID) for this task.
 */
export default function ApiWorkflowRelationEntry(props) {
  const { element } = props;
  ...
  const [items, setItems] = useState(() => getApiWorkflowRelations(elementId));
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const refresh = () => setItems(getApiWorkflowRelations(elementId));
  ...
  const handleDelete = useCallback(async (callRefId) => {
    setLoading(true);
    try {
      await deleteApiWorkflowRelation(elementId, callRefId);
    } finally {
      setLoading(false);
    }
  }, [elementId]);
  ...
  if (!items || items.length === 0) {
    return html`
      <div class="bio-properties-panel-entry" style="padding: 8px 12px; color: #888; font-size: 12px;">
        ${translate('No API workflow relations')}
      </div>
    `;
  }
