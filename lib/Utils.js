...
function createDropdown(dropdown) {
  ...
  var dropdownNode = domify(
    '<div class="group__dropdown">' +
      ...
      '</button>' +
      '<div class="group__dropdown-menu group__dropdown-menu--hidden"></div>' +
    '</div>'
  );
  ...
  forEach(menu, function(menuItem) {
    var menuItemNode = domify('<div class="group__dropdown-menu-item">' + escapeHTML(menuItem.label) + '</div>');
