...
function getFormControlValuesInScope(entryNode) {
  ...
  forEach(controlNodes, function(controlNode) {
    var value, optionsArray, selectedOptionValues;
    if (controlNode.localName === 'select' && controlNode.multiple === true) {
      optionsArray = Array.from(controlNode.querySelectorAll('option'));
      ...
      value = selectedOptionValues.join(', ');
    } else {
...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  domDelegate.bind(container, '.properties-tabs [data-tab-target]', 'click', function(event) {
...
// Applies custom options to an entry dom element.
function applyCustomOptions(entry, entryNode, customOptions) {
  if (customOptions) {
    if (customOptions.classes) {
      forEach(customOptions.classes, function(classList, selector) {
        var elem;
        if (selector.length === 0) {
          elem = entryNode;
        } else {
          var elem = entryNode.querySelector(selector);
        }
        if (elem) {
          forEach(classList || [], function(cssClass) {
            domClasses(elem).add(cssClass);
          });
        }
      });
    }
    if (customOptions.afterRender && typeof customOptions.afterRender === "function")
      customOptions.afterRender(entry, entryNode);
  }
}
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  var panelNode = domify('<div class="bpp-properties"></div>'),
      headerNode = domify('<div class="bpp-properties-header">' +
        ...
        '</div>' +
      '</div>'),
      tabBarNode = domify('<div class="properties-tabs-container"></div>'),
      tabLinksNode = domify('<ul class="properties-tabs-list"></ul>');

  if (self.config.customOptions && self.config.customOptions.header) {
    applyCustomOptions({}, headerNode, self.config.customOptions.header)
  }
  ...
  forEach(tabs, function(tab, tabIndex) {
    ...
    var tabNode = domify('<div class="properties-tab-panel" data-tab="' + tab.id + '"></div>'),
        tabLinkNode = domify('<li class="properties-tab-list-item">' +
          '<div class="properties-tab">' +
            '<input type="checkbox" checked>' +
            '<i></i>' +
            '<h6 data-tab-target="' + tab.id + '">' + tab.label + '</h6>' +
          '</div>' +
        '</li>');
    ...
    forEach(groups, function(group) {
      ...
      var groupNode = domify('<div class="bpp-properties-group" data-group="' + group.id + '">' +
          '<span class="group-toggle bpp-hidden"></span>' +
      ...
      if (self.config.customOptions && self.config.customOptions.groups) {
        applyCustomOptions(group, groupNode, self.config.customOptions.groups.common);
      }
      ...
      forEach(group.entries, function(entry) {
        ...
        // if there are common, or entry specific custom options, we apply them
        if (self.config.customOptions && self.config.customOptions.entries) {
          applyCustomOptions(entry, entryNode, self.config.customOptions.entries.common);
          applyCustomOptions(entry, entryNode, self.config.customOptions.entries[entry.id]);
        }
      });
    ...
    tabLinkNode.querySelector('.properties-tab').appendChild(tabNode);
