...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  var handleChange = function handleChange(event) {
    ...
    // Check whether a 'data-no-change' attribute exist in the HTML template to prevent
    // to call the 'set' function. If this attribute is set, no changes are made to the
    // business object. Instead it is possible to simply change the content of DOM elements
    // in the respective entry.
    var noChanges = event.delegateTarget.hasAttribute('data-no-change');
    if (noChanges) {
      self.update(self._current.element);
      return;
    }
