...
PropertiesPanel.prototype.updateShow = function(entry, node) {
  ...
  forEach(showNodes, function(showNode) {
    ...
    if (fn) {
      ...
      var hasClass = domClasses(showNode).has('pp-hidden');
      if (shouldShow) {
        if (hasClass) {
          domClasses(showNode).remove('pp-hidden');
        }
      } else {
        domClasses(showNode).add('pp-hidden');
      }
    }
  });
};
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  forEach(tabs, function(tab, _t) {
    ...
    domQuery('a', tabLinkNode).addEventListener('click', function (evt) {
      ...
      if (domClasses(evt.target).contains('active')) { return; }

      var id = evt.target.getAttribute('data-tab-target');

      // remove "active" classes
      forEach(domQuery.all('a', tabLinksNode), function (el) {
        domClasses(el.parentNode).remove('active');
      });

      forEach(domQuery.all('.djs-properties-tab[data-tab]', tabContainerNode), function (el) {
        domClasses(el).remove('active');
      });

      // add "active" classes
      domClasses(domQuery('.djs-properties-tab[data-tab="'+ id +'"]')).add('active');
      domClasses(evt.target.parentNode).add('active');
    });

    if (!_t) {
      tabNode.classList.add('active');
      tabLinkNode.classList.add('active');
    }
