...
var tabHelper = require('./helper/TabHelper');
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  var tabLinksNode = domQuery('.djs-properties-tabs-links', panelNode);

  // If not all tabs fit in the tab bar, show scroll buttons
  if (tabLinksNode.scrollWidth > tabLinksNode.offsetWidth) {
      domClasses(domQuery('.tabs-scroll-left', panelNode)).remove(HIDE_CLASS);
      domClasses(domQuery('.tabs-scroll-right', panelNode)).remove(HIDE_CLASS);
  }
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  var scrollLeftButton = domify('<span class="tabs-scroll-left ' + HIDE_CLASS + '">&lsaquo;</span>'),
      scrollRightButton = domify('<span class="tabs-scroll-right ' + HIDE_CLASS + '">&rsaquo;</span>');

  scrollLeftButton.addEventListener('click', function(event) {
    tabHelper.switchToAdjacentTab(-1, panelNode);
  });
  ...
  forEach(tabs, function(tab, _t) {
    ...
    domQuery('a', tabLinkNode).addEventListener('click', function(evt) {
      tabHelper.switchToTab(evt.target, panelNode);
      evt.preventDefault();
    });
  ...
  // add scrolling over tabs
  tabLinksNode.addEventListener('wheel', function(e) {
    tabHelper.switchToAdjacentTab(e.deltaY, panelNode);
    e.preventDefault();
  }, false);

  tabBarNode.appendChild(scrollRightButton);
  tabBarNode.appendChild(scrollLeftButton);
