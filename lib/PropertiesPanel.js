 ...
 */
function PropertiesPanel(config, eventBus, modeling, propertiesProvider) {
...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  domDelegate.bind(container, '[data-action]', 'click', function onClick(event) {
    ...
    self.executeAction(entry, actionId);
  });
};

PropertiesPanel.prototype.executeAction = function(entry, actionId) {
  var current = this._current;
  ...
  if (actionId in entry) {
    entry[actionId](current.element);
  }
};

PropertiesPanel.prototype.applyChanges = function(entry, values) {

  var element = this._current.element;

  var actualChanges = entry.set(element, values);
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  forEach(current.groups, function(group) {
    ...
    forEach(group.entries, function(entry) {
      ...
      var values = 'get' in entry ? entry.get(element) : {};
      ...
      forEach(inputNodes, function(node) {
        ...
        if ('value' in node) {
          ...
          if (isToggle(node)) {
            if (node.value == value || (!domAttr(node, 'value') && value)) {
              node.checked = true;
            } else {
              node.checked = false;
            }
