...
function isDetached(node) {
  ...
  while (node) {
    ...
    // we are attached to the document (!)
...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  var handleChange = function handleChange(event) {
    ...
    // discard out of band updates from already detached
    // nodes. this may may happen due to debounce
    if (isDetached(node)) {
      return;
    }
  ...
  domDelegate.bind(container, '[data-blur]', 'input', debounce(handleInput, DEBOUNCE_DELAY), true);
