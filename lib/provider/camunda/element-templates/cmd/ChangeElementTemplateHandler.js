...
function ChangeElementTemplateHandler(modeling, commandStack, bpmnFactory) {
  ...
  function updateProperties(element, newTemplate) {
    ...
    var newProperties = createPropertyUpdates(newTemplate, bpmnFactory);
...
function createPropertyUpdates(template, bpmnFactory) {
...
function createInputOutputMappings(template, bpmnFactory) {
  ...
  template.properties.forEach(function(p) {
    var binding = p.binding,
        ...
        scriptFormat = binding.scriptFormat,
        parameterValue,
        parameterDefinition,
        parameter;
    ...
    if (bindingType === 'camunda:inputParameter') {
      ...
      if (scriptFormat) {
        ...
        parameterValue = binding.value;
      }

      parameter = bpmnFactory.create('camunda:InputParameter', {
        name: binding.target,
    ...
    if (bindingType === 'camunda:outputParameter') {
      ...
      parameter = bpmnFactory.create('camunda:OutputParameter', {
        name: p.value,
        value: parameterValue,
        definition: parameterDefinition
      });
