...
function propertyChanged(element, oldProperty) {
  ...
  if (oldBindingType === 'property') {
    if (oldBindingName === 'conditionExpression') {
      ...
      return conditionExpression.get('bpmn:body') !== oldValue;
    }

    return businessObject.get(oldBindingName) !== oldValue;
  }

  if (oldBindingType === 'camunda:field') {
    return businessObject.get('camunda:string') !== oldValue;
  }

  if (oldBindingType === 'camunda:in') {
    if (oldBinding.expression) {
      return businessObject.get('sourceExpression') !== oldValue;
    } else {
      return businessObject.get('camunda:source') !== oldValue;
    }
  }

  if (oldBindingType === 'camunda:in:businessKey') {
    return businessObject.get('camunda:businessKey') !== oldValue;
  }

  if (oldBindingType === 'camunda:out') {
    return businessObject.get('camunda:target') !== oldValue;
  }
}