...
module.exports = function(element, elementTemplates, bpmnFactory, translate) {
  ...
  var renderCustomField = function(id, p, idx, scopeIdx) {
  ...
  if (template.scopes) {
    template.scopes.forEach(function(scope, scopeIdx) {

      var customScopeFieldsGroup = {
        id: 'customField-' + scopeIdx,
        label: translate('Custom Fields for scope: ') + scope.scope,
        entries: []
      };

      scope.properties.forEach(function(p, idx) {

        id = 'custom-' + template.id + '-' + scopeIdx + '-' + idx;

        entry = renderCustomField(id, p, idx, scopeIdx);
        if (entry) {
...
function getPropertyValue(element, property) {
  ...
  var inside = false;
  if (bindingScope) {
    bo = findExtension(bo, bindingScope.name);
    inside = bindingScope.inside;
  }
