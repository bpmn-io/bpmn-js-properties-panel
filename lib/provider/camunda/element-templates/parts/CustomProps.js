...
module.exports = function(element, elementTemplates, bpmnFactory, translate) {
  ...
  var renderCustomField = function(id, p, idx, scopeName) {
  ...
  if (template.scopes) {
    for (var scopeName in template.scopes) {
      ...
      scope.properties.forEach(function(p, idx) {
        ...
        id = 'custom-' + template.id + '-' + idScopeName + '-' + idx;

        entry = renderCustomField(id, p, idx, scopeName);
        if (entry) {
...
function getPropertyValue(element, property) {
  ...
  if (bindingScope) {
    ...
    insideProperties = bindingScope.insideProperties;
  }
