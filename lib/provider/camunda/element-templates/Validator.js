...
function Validator() {
  ...
  this._validateTemplate = function(template) {
    ...
    if (scopes) {
      if (!isArray(scopes)) {
        err = this._logError('invalid scopes, should be scopes=[]', template);
      }
      else {
        var _this = this;
        scopes.forEach(function(scope) {
          if (!isObject(scope)) {
            err = _this._logError('invalid scope, should be scope={}', template);
          }

          if (!scope.name) {
            err = _this._logError('missing scope name', template);
          }

          if (!isArray(scope.properties)) {
            err =  _this._logError('missing properties=[] in scope <' + scope.name + '>', template);
          } else {
            if (!_this._validateProperties(scope.properties)) {
              err = new Error('invalid properties in scope <' + scope.name + '>');
            }
          }
        });
      }
