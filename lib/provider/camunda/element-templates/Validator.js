    ...
    if (scopes) {
      if (!isArray(scopes)) {
        ...
        scopes.forEach(function(scope) {
          ...
          if (!scope.name) {
            err = self._logError('missing scope name', template);
          }

          if (!isArray(scope.properties)) {
            err =  self._logError('missing properties=[] in scope <' + scope.name + '>', template);
          } else {
            if (!self._validateProperties(scope.properties)) {
