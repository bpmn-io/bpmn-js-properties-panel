...
function getProperty(template, entry) {
  ...
  if (idx.indexOf('-') !== -1) {
    ...
    if (indexes.length == 2) {
      var scopeIndex = parseInt(indexes[0], 10);
      index = parseInt(indexes[1], 10);
      if (!isNaN(scopeIndex) && !isNaN(index)) {
        var prop = template.scopes[scopeIndex].properties[index];
        // Add scope information
        prop.binding.scope = {
          name: template.scopes[scopeIndex].scope,
          inside: template.scopes[scopeIndex].scope || false
        };
        return prop;
      }
    }
