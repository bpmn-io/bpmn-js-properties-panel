...
module.exports = function(group, element, bpmnFactory) {
  ...
  group.entries.push({
    id: 'connector-id-error',
    html: '<div data-value></div>',
    setControlValue: function(element, entryNode, node) {
      domClasses(node).remove('pp-hidden');
      domClasses(node).remove('pp-error-message');
      node.textContent = '';

      if (getImplementationType(element) === 'connector') {
        var connector = InputOutputHelper.getConnector(element);
        var connectorId = connector.get('connectorId');

        if (!connectorId) {
          domClasses(node).add('pp-error-message');
          node.textContent = 'Must provide a connector id';
          return;
        }
      }
      domClasses(node).add('pp-hidden');
    }
  });
