...
module.exports = function(group, element, bpmnFactory) {
  ...
  var newElement = function(type, prop) {
    ...
    return function(element, extensionElements, value) {
      ...
      var inputOutput = (getElements(getBusinessObject(element), 'camunda:InputOutput') || [])[0];
      if (!inputOutput) {
      ...
      var newElem = createParameter(type, inputOutput, { name: value });
  ...
  group.entries.push(entryFactory.validationAwareTextField({
    ...
    getProperty: function(element, node) {
      return (getSelection(element, node) || {}).name;
    },
