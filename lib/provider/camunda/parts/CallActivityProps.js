...
function getCallActivityType(node) {
  var callActivityType = domQuery('select[name=callActivityType] > option:checked', node.parentElement);

  if (callActivityType === null) {
    callActivityType = domQuery('select[name=callActivityType] > option[selected=selected]', node.parentElement);
  }
...
module.exports = function(group, element) {
  ...
  group.entries.push({
    ...
    isBPMN: function(element, node) {
      ...
      return callActivityType && callActivityType.value === 'bpmn';
    },
    isCMMN: function(element, node) {
      var callActivityType = getCallActivityType(node);

      return callActivityType && callActivityType.value === 'cmmn';
    },
