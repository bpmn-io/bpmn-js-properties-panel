...
module.exports = function (element, bpmnFactory, params) {
  ...
  return {
    id: id,
    html: '<div class="pp-row" id="cam-extension-elements-wrapper' + (id ? '-' + id : '') + '" ' +
    ...
    set: function(element, values, node) {
      ...
      if (action.id === 'create-element') {
        ...
        if (typeof newElement === 'function') {
          commands.push(newElement(element, extensionElements, action.value));
        }
      }
    ...
    isDisabled: function(element, node) {
      if (canBeDisabled && typeof canBeDisabled === 'function') {
        return !canBeDisabled(element, node);
      }
    },
