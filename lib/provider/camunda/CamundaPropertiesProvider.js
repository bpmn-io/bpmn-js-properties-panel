...
var elementTemplateDescriptionProps = require('./element-templates/parts/DescriptionProps'),
    ...
    getTemplate = require('./element-templates/Helper').getTemplate,
...
function createElementTemplateGroups(element, bpmnFactory, canvas, elementTemplates, modeling, replace, selection, translate) {
  ...
  if (!templateId || !getTemplate(element, elementTemplates)) {
    return [];
  }
  ...
  return [ descriptionGroup ].concat(customFieldsGroups);
}
...
function createProcessVariablesTabGroups(element, translate) {
  var processVariablesGroup = {
    ...
    label : translate('Available Process Variables'),
    entries: []
  };
